...
async def create_polluter(
    polluter: Polluter_OO_Create = Body(),
    SessionLocal: AsyncSession = Depends(get_db_session)
    ) -> API_Response:

    polluter = Models.Polluter_OO(**polluter.model_dump()) # unpacking poles is safe, due undeclaired (im schema) poles are ignored by default: https://pydantic.dev/docs/validation/latest/api/pydantic/config/
...
async def add_polluter_wastes(
   polluter_waste: PolluterWaste_Create = Body(),
   SessionLocal: AsyncSession = Depends(get_db_session) 
   ) -> API_Response:
    ...
    if polluter_waste.amount < 1: return unpositive_amount_handler(polluter_waste.amount) 
    category_handler(polluter_waste.category) # Rise HTTP exception 409 if chosen category doesn`t excists
    
    polluter_waste = Models.PolluterWaste(**polluter_waste.model_dump())
...
@router.put('/{polluter_id}', status_code=200, summary='(TODO - Polluter Auth) | Manual modify polluter')
async def correct_coords(
    polluter_id: str,
    polluter: Polluter_OO_Modify = Body(description='poles to modify')
)-> API_Response:
    ''' Update Polluter. Checks if Polluter has any active release contracts before modifying coords (affects carring distance) '''
    async with sessionFactory() as session:
        # check if some waste is contracted => can`t modify coords
        ...
        if contracted_wastes: return Response(status_code=412, content=f'Can`t modify polluter due it`s already contracted to release. For ex see contract_id: {contracted_wastes}')

        # absorb all requested poles to redo 
        data_to_update = polluter.model_dump(exclude_unset=True)
        if not data_to_update:
            return Response(status_code=204)
    
        stmt = (
            update(Models.Polluter_OO)
            .where(Models.Polluter_OO.id == polluter_id)
            .values(**data_to_update)
            .returning(Models.Polluter_OO)      # configure to return row - instead of just ammount of updated rows
        ) # update only passed poles

        row = await session.execute(stmt)
        row = row.scalar_one_or_none()
        if not row:
            return Response(status_code=409, content=f'no such id: {polluter_id}')
        
        await session.commit()      # prevent silent rollback() on error 

        return API_Response(
            **common_statuses[200]['OK'],
            data=[convert_to_pydentic(row, Polluter_OO)]
            )
...
async def correct_amount(
    polluter_waste_id: str,
    polluter_waste: PolluterWaste_Modify = Body(description='poles to modify')
) -> API_Response:
    ...
    async with sessionFactory() as session:
        if polluter_waste.amount:
            if polluter_waste.amount == 0: return Response(status_code=409, content='to delete resourse - use DELETE method')  # special case
            if polluter_waste.amount < 1: return unpositive_amount_handler(polluter_waste.amount)

        row = await session.execute(select(Models.PolluterWaste).where(Models.PolluterWaste.id == polluter_waste_id))        
        row = row.scalar_one_or_none()
        if not row:
            return Response(status_code=409, content=f'no such id: {polluter_waste_id}')
        ...
        # absorb all requested poles to redo 
        data_to_update = polluter_waste.model_dump(exclude_unset=True)
        if not data_to_update:
            return Response(status_code=204)

        stmt = (
            update(Models.PolluterWaste)
            .where(Models.PolluterWaste.id == polluter_waste_id)
            .values(**data_to_update)
            .returning(Models.PolluterWaste)  # configure to return row - instead of just ammount of updated rows
        ) # update only passed poles
        ...
        return API_Response(
            **common_statuses[200]['OK'],
            data=[convert_to_pydentic(row, PolluterWaste)]
            )
